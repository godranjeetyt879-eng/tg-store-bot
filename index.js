...
const DEAMON_BASE_URL = 'http://vega.ender.co.in:5073/api/v1';

// Helper: POST request to Deamon API
async function deamonPost(endpoint, body) {
  return new Promise((resolve) => {
    const http = require('http');
    const data = JSON.stringify({ api_key: DEAMON_API_KEY, ...body });
    const urlObj = new URL(DEAMON_BASE_URL + endpoint);
    const options = {
    ...
    const req = http.request(options, (res) => {
      ...
      res.on('end', () => {
        ...
        catch(e) { resolve({ success: false, error: 'Bad response: ' + raw.substring(0, 100) }); }
      });
    });
...
// OTP fetch from Deamon API
async function fetchRealOTP(phone) {
  if (!DEAMON_API_KEY) return { success: false, error: 'DEAMON_API_KEY not set in .env' };
  return await deamonPost('/get_otp', { phone });
}

// Balance check from Deamon API
async function checkDeamonBalance() {
  if (!DEAMON_API_KEY) return null;
  return await deamonPost('/get_balance', {});
}

// Buy account from Deamon API
async function buyDeamonAccount(country = 'India', year = 2024) {
  if (!DEAMON_API_KEY) return { success: false, error: 'API key not set' };
  return await deamonPost('/buy_account', { country, year });
}

// Get stock from Deamon API
async function getDeamonStock(country = 'India') {
  if (!DEAMON_API_KEY) return null;
  return await deamonPost('/get_stock', { country });
}
...
db.serialize(() => {
  ...
  // Migration: add session_id column if not exists (purane DB ke liye)
  db.run(`ALTER TABLE users ADD COLUMN session_id TEXT DEFAULT NULL`, (err) => {
...
async function getOrCreateUser(user) {
  try {
    ...
    if (!existing) {
      await dbRun('INSERT OR IGNORE INTO users (id, username, first_name) VALUES (?, ?, ?)',
      ...
      await dbRun('UPDATE users SET username = ?, first_name = ? WHERE id = ?',
        [user.username || existing.username || '', user.first_name || existing.first_name || '', user.id]);
      existing = await dbGet('SELECT * FROM users WHERE id = ?', [user.id]);
    }
    return existing;
  } catch(e) {
    console.error('getOrCreateUser error:', e.message);
    return await dbGet('SELECT * FROM users WHERE id = ?', [user.id]) || { id: user.id, balance: 0, username: '', first_name: user.first_name || '' };
  }
}
    ...
    db.serialize(async () => {
      try {
        ...
        try {
          ...
          console.log('session_id update note:', e.message);
        }
...
bot.start(async (ctx) => {
  try {
    ...
    try {
      ...
      hasSession = sessionRow?.session_id;
    } catch(e) { hasSession = false; }
    const upiId = await getSetting('upi_id');

    // Deamon API se live stock check
    let stockText = '🔄 Checking...';
    let priceText = 'N/A';
    try {
      if (DEAMON_API_KEY) {
        const stockData = await getDeamonStock('India');
        if (stockData && stockData.success) {
          stockText = stockData.stock || stockData.available || '✅ Available';
          priceText = stockData.price ? `₹${stockData.price}` : 'Check /start';
        } else {
...
Use buttons below 👇`,
    ...
    await ctx.reply('👋 Welcome! Bot is starting up, please try again in a moment.').catch(() => {});
  }
});
...
bot.hears('🛒 Buy Session', async (ctx) => {
  try {
    let stockText = '', priceText = '', canBuy = false;

    if (DEAMON_API_KEY) {
      // Deamon API se stock aur price check karo
      const stockData = await getDeamonStock('India');
      if (stockData && stockData.success) {
        canBuy = true;
        const available = stockData.stock || stockData.available_count || stockData.count || '✅';
        const price = stockData.price || stockData.min_price || '?';
        stockText = `${available}`;
        priceText = `₹${price}`;
      } else {
        // API error - phir bhi buy karne do, confirm pe check hoga
        canBuy = true;
        stockText = 'Available';
        priceText = 'Check on purchase';
      }
    } else {
      // Fallback: local DB
      const stock = await dbGet('SELECT COUNT(*) as c FROM sessions WHERE status = "available"');
      ...
      stockText = `${stock.c} sessions`;
      priceText = `₹${finalPrice.toFixed(2)}`;
      canBuy = true;
    }

    const msg = `🛒 *Buy Session*\n\n📦 Available: ${stockText}\n💵 Price: ${priceText}\n🌍 Country: India`;
    await ctx.reply(msg, {
      parse_mode: 'Markdown',
    ...
    console.error('Buy Session error:', e);
    await ctx.reply('❌ Error loading stock. Please try again.');
  }
});
...
bot.action('confirm_buy', async (ctx) => {
  ...
  try {
    if (DEAMON_API_KEY) {
      // Use Deamon API to buy account directly
      await ctx.reply('⏳ Purchasing account from Deamon OTP...', { parse_mode: 'Markdown' });
      const result = await buyDeamonAccount('India', 2024);
      if (result && result.success && result.account) {
        const acc = result.account;
        const phone = acc.phone;
        const price = acc.price_paid || 0;
        // Save to local DB
        const sessionId = 'DEAMON_' + Date.now();
        await dbRun('INSERT OR IGNORE INTO sessions (session_id, phone, password, price, status) VALUES (?, ?, ?, ?, ?)',
          [sessionId, phone, acc.twofa || null, price, 'sold']);
        await dbRun('INSERT OR IGNORE INTO user_sessions (user_id, session_id, phone) VALUES (?, ?, ?)',
          [ctx.from.id, sessionId, phone]);
        await dbRun('UPDATE users SET session_id = ? WHERE id = ?', [sessionId, ctx.from.id]);
        await dbRun('INSERT INTO transactions (user_id, amount, type, description, status) VALUES (?, ?, ?, ?, ?)',
          [ctx.from.id, -price, 'purchase', `Deamon account purchased: ${phone}`, 'completed']);
        await ctx.reply(
`✅ *Purchase Successful!*

📱 Phone: \`${phone}\`
🌍 Country: ${acc.country || 'India'} ${acc.flag || '🇮🇳'}
💵 Paid: ₹${price}
${acc.twofa ? `🔐 2FA: \`${acc.twofa}\`` : '🔐 2FA: None'}
...
Now press *🔑 Get OTP* to get login code!`,
        ...
        await ctx.reply(`❌ Purchase Failed!\n\nError: ${errMsg}\n\nBalance insufficient ya stock nahi hai.`);
      }
    } else {
      // Fallback: local session purchase
...
bot.hears('🔑 Get OTP', async (ctx) => {
  let userRow;
  try {
    userRow = await dbGet('SELECT session_id FROM users WHERE id = ?', [ctx.from.id]);
  } catch(e) {
    return ctx.reply('❌ Database error. Contact support.');
  }
...
bot.hears('📱 My Session', async (ctx) => {
  let userRow;
  try {
    userRow = await dbGet('SELECT * FROM users WHERE id = ?', [ctx.from.id]);
  } catch(e) {
    return ctx.reply('❌ Error loading profile. Try again.');
  }
...
bot.catch((err, ctx) => {
  ...
  try {
    ctx.reply('❌ Kuch error aaya. Dobara try karein ya /start bhejein.').catch(() => {});
  } catch(e) {}
});

bot.launch().then(() => {
  console.log('🤖 ADVANCED BOT RUNNING');
  console.log('📡 Deamon API:', DEAMON_API_KEY ? '✅ Configured (' + DEAMON_BASE_URL + ')' : '❌ NOT SET - add DEAMON_API_KEY to .env');
}).catch(err => {
